...
do i=1,17
  write(6,'(A10,2X,5ES11.3)') spec(i),Nwrite(i),Tex(i,itot),tau(i,itot),tau_test(i),tr_incr(itot-2,i)*c**2/2./kb/freq0(i)**2
enddo
...
!write(6,*) 'give prefix'
!read(5,*) prefix
filepdr=trim(adjustl(prefix))//".pdr.fin"
fileline=trim(adjustl(prefix))//".line.fin"
filetau=trim(adjustl(prefix))//".opdp.fin"
filepop=trim(adjustl(prefix))//".spop.fin"
